...
import { onMount } from 'svelte';
	...
	const commands = $derived([
		...
		...journalEntries.map((e: { id: string; title: string }) => ({ type: 'journal', id: e.id, title: `Open entry: ${e.title}` }))
	]);
...
{#if isOpen}
	<!-- Overlay (later in DOM so it paints above without z-index) -->
	<div class="fixed inset-0 bg-black/60 backdrop-blur-sm" role="dialog" aria-modal="true" tabindex="-1" onclick={close} onkeydown={(e: KeyboardEvent) => { if (e.key === 'Escape') close(); }}></div>
	<div class="fixed top-10 left-1/2 w-[min(680px,92vw)] -translate-x-1/2 rounded-2xl border border-white/10 bg-black/80 p-3 shadow-xl" role="dialog" aria-modal="true" tabindex="0">
		<div class="flex items-center gap-2 rounded-lg border border-white/10 bg-black/60 px-3 py-2">
			<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="text-white/60"><circle cx="11" cy="11" r="8"></circle><path d="m21 21-4.3-4.3"></path></svg>
			<input id="cmdk-input" class="flex-1 bg-transparent text-sm text-white outline-none placeholder:text-white/40" placeholder="Search (tabs, journal entries)..." bind:value={query} onkeydown={(e: KeyboardEvent) => {
			...
			{:else}
				<ul>
					{#each filtered as item, i}
						<li>
							<button class="w-full text-left px-4 py-2 text-sm transition-colors {i === activeIndex ? 'bg-white/10 text-white' : 'text-white/80 hover:bg-white/5'}" onclick={() => submit(item)}>
								{item.title}
							</button>
						</li>
					{/each}
