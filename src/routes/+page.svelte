    ...
    {#if mounted}
      ...
      <div class="relative z-10 min-h-screen pt-16">
        ...
        <div class="w-full max-w-3xl mx-auto mb-4 px-3 md:px-6">
          <div class="bg-black/30 backdrop-blur-md border border-accent/20 rounded-xl p-1.5 grid grid-cols-4 gap-1 shadow-lg shadow-accent/5">
            <button 
            ...
            <button 
              ...
              class="py-3 px-4 text-center rounded-lg transition-all duration-300 font-handwriting relative overflow-hidden {selectedTab === 'book' 
                ? 'bg-gradient-to-br from-black/80 to-black/60 text-accent font-bold shadow-inner' 
                : 'text-white hover:text-accent hover:bg-black/40'}"
            >
          ...
          {:else if selectedTab === 'book'}
            <div in:fade={{ duration: 300 }} class="flex flex-col items-center">
              <div class="w-full max-w-3xl mx-auto">
                <div class="bg-black/40 rounded-2xl overflow-hidden border border-accent/20 backdrop-blur-sm book-container">
                  <!-- Chapter Navigation -->
                  <div class="p-4 border-b border-accent/10 flex justify-between items-center">
                    <button 
                      onclick={() => {
                        currentChapter = Math.max(0, currentChapter - 1);
                        document.querySelector('.book-container')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                      }}
                      class="px-4 py-2 text-white hover:text-accent transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    ...
                    <span class="text-accent font-handwriting text-lg">{bookChapters[currentChapter].title}</span>
                    <button 
                      onclick={() => {
                  ...
                  <!-- Chapter Content -->
                  <div class="p-6 md:p-8 chapter-content">
                    <div class="prose prose-invert max-w-none">
                      <div class="whitespace-pre-line text-white/90 font-handwriting leading-relaxed">
                  ...
                  <div class="p-4 border-t border-accent/10 flex justify-between items-center">
                    <button 
                      onclick={() => {
                        currentChapter = Math.max(0, currentChapter - 1);
                        document.querySelector('.book-container')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
