		...
		<div class="manga-panel relative p-5" style="border-radius: var(--radius)">
			{#if now?.item}
				<div class="flex items-center gap-4">
					<img
						src={now.item.album?.images?.[2]?.url || now.item.album?.images?.[0]?.url}
						alt="art"
				...
				<a
					class="manga-panel absolute top-3 right-3 inline-flex items-center gap-1 px-3 py-1 text-[11px] no-underline opacity-100"
					style="border-radius: var(--radius)"
					href={now.item.external_urls?.spotify}
					target="_blank"
					rel="noreferrer"
				...
				<div class="grid grid-cols-1 gap-3">
					{#each recent.items.slice(0, 10) as it}
						<div class="flex items-center gap-3">
							<img
								src={it.track.album?.images?.[2]?.url || it.track.album?.images?.[0]?.url}
								alt="art"
								class="h-10 w-10 object-cover ring-1 ring-black"
							/>
							<div>
